...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        // Update the record
        $stmt = $pdo->prepare('UPDATE users SET id = ?, username = ?, email = ?, password = ?WHERE id = ?');
        $stmt->execute([$id, $username, $email, $password, $_GET['id']]);
...
<div class="content update">
	<h2>Update Data Users #<?=$contact['id']?></h2>
